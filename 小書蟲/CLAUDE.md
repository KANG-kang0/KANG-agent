...
## 踩過的坑
- 【坑】用 preview 工具起本機 server 時，沙盒可能擋 `os.getcwd()` 且讀不到專案目錄（回 404）。改用 launch.json 裡的 python 一行式（已寫好），還是 404 就把 `小書蟲-web/` 複製到 scratchpad 再把 directory 指過去。（2026-07-07）
